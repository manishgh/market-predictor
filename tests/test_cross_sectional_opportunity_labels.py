...
from market_predictor.research.intraday_cross_sectional.opportunity_labels import OpportunityLabelConfig, build_opportunity_labels


class CrossSectionalOpportunityLabelTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.config = OpportunityLabelConfig(
            horizons_bars=(1, 2),
    ...
    def test_exact_interval_cost_benchmark_and_path_math(self) -> None:
        labeled = build_opportunity_labels(self.bars, self.benchmarks, config=self.config)
    ...
    def test_labels_never_cross_eastern_sessions(self) -> None:
        ...
        labeled = build_opportunity_labels(bars, benchmarks, config=self.config)
    ...
    def test_development_builder_rejects_shadow_rows(self) -> None:
        ...
        with self.assertRaises(LeakageAuditError):
            build_opportunity_labels(bars, self.benchmarks, config=self.config)
    ...
    def test_missing_exact_benchmark_bar_fails_closed(self) -> None:
        ...
        with self.assertRaises(DataReadinessError):
            build_opportunity_labels(self.bars, benchmarks, config=self.config)
    ...
    def test_ticker_gap_drops_non_exact_horizon(self) -> None:
        ...
        labeled = build_opportunity_labels(bars, self.benchmarks, config=self.config)
        ...
        self.assertFalse(bool(((labeled["ticker"] == "AAA") & (labeled["decision_time_utc"] == self.times[0])).any()))
    ...
    def test_regular_session_decisions_never_use_after_hours_bars(self) -> None:
        ...
        labeled = build_opportunity_labels(bars, benchmarks, config=config)
