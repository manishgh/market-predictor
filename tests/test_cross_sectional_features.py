...
class CrossSectionalFeatureTests(unittest.TestCase):
    ...
    def test_cross_sectional_ranks_use_only_current_decision_group(self) -> None:
        features = build_cross_sectional_features(self.bars, self.benchmarks, minimum_cross_section=3)
    ...
    def test_future_bars_cannot_change_prior_features(self) -> None:
        baseline = build_cross_sectional_features(self.bars, self.benchmarks, minimum_cross_section=3)
        changed = self.bars.copy()
        changed.loc[changed["timestamp"] == self.times[-1], ["high", "low", "close", "volume"]] = [900, 1, 500, 9_000_000]
        rescored = build_cross_sectional_features(changed, self.benchmarks, minimum_cross_section=3)
    ...
    def test_batch_and_truncated_live_history_have_feature_parity(self) -> None:
        batch = build_cross_sectional_features(self.bars, self.benchmarks, minimum_cross_section=3)
        cutoff = self.times[5]
        live = build_cross_sectional_features(
    ...
    def test_missing_microstructure_is_explicit_and_not_in_core(self) -> None:
        ...
        self.assertTrue(set(MICROSTRUCTURE_FEATURE_COLUMNS).isdisjoint(core_feature_columns()))
    ...
    def test_returns_reset_at_session_boundary(self) -> None:
        ...
        features = build_cross_sectional_features(bars, benchmarks, minimum_cross_section=3)
    ...
    def test_missing_exact_sector_bar_fails_closed(self) -> None:
        benchmarks = self.benchmarks[~((self.benchmarks["ticker"] == "XLK") & (self.benchmarks["timestamp"] == self.times[3]))]
        with self.assertRaises(DataReadinessError):
            build_cross_sectional_features(self.bars, benchmarks, minimum_cross_section=3)
    ...
    def test_feature_build_performance_smoke(self) -> None:
        ...
        features = build_cross_sectional_features(bars, self.benchmarks, minimum_cross_section=20)
        elapsed = perf_counter() - started
        self.assertEqual(len(features), len(bars))
        self.assertLess(elapsed, 8.0, f"480-row cross-sectional feature smoke build took {elapsed:.2f}s")

    def test_sharded_feature_stages_match_single_pass(self) -> None:
        expected = build_cross_sectional_features(self.bars, self.benchmarks, minimum_cross_section=3)
        shards = [build_ticker_features(group) for _, group in self.bars.groupby("ticker", sort=False)]
        actual = finalize_cross_sectional_features(
