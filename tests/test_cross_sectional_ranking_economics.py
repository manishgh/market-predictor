...
class CrossSectionalRankingEconomicsTests(unittest.TestCase):
    ...
    def test_ranker_scores_cannot_be_mislabeled_as_probabilities(self) -> None:
        with self.assertRaises(DataReadinessError):
            fit_disjoint_calibrator(self.predictions, family="xgboost_ranker")

    def test_session_blocked_ranking_audit_is_reproducible(self) -> None:
        _, _, calibration = fit_disjoint_calibrator(self.predictions, family="downside_classifier", minimum_sessions=6)
        evidence = build_multi_output_evidence(
            self.predictions,
            opportunity_family="xgboost_ranker",
    ...
    def test_raw_downside_scores_cannot_drive_production_economics(self) -> None:
        evidence = build_multi_output_evidence(self.predictions, opportunity_family="xgboost_ranker")
        with self.assertRaises(DataReadinessError):
            evaluate_ranking_economics(
                evidence,
    ...
    def test_overlapping_decision_groups_are_not_double_counted(self) -> None:
        ...
        overlapping["primary_exit_time_utc"] = pd.to_datetime(overlapping["primary_exit_time_utc"], utc=True) + pd.Timedelta(minutes=5)
