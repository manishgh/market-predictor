...
class CrossSectionalCatalystOverlayTests(unittest.TestCase):
    def test_catalyst_joins_only_available_events_and_deduplicates_directories(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            first.mkdir()
            second.mkdir()
            _events("AAA", sentiment=0.9, available_at="2026-01-05T13:30:00Z").to_parquet(first / "AAA_events.parquet", index=False)
            _events("AAA", sentiment=0.9, available_at="2026-01-05T13:30:00Z").to_parquet(second / "AAA_events.parquet", index=False)
            _events("BBB", sentiment=-0.9, available_at="2026-01-05T13:35:00Z", offering=True).to_parquet(
                first / "BBB_events.parquet", index=False
            )
            _events("CCC", sentiment=0.2, available_at="2026-01-05T14:30:00Z").to_parquet(first / "CCC_events.parquet", index=False)
            evidence, audit = build_catalyst_overlay_evidence(
                _predictions(),
        ...
        first_group = evidence[
            evidence["decision_time_utc"].eq(pd.Timestamp("2026-01-05T14:00:00Z")) & evidence["audit_scope"].eq("walk_forward")
        ].set_index("ticker")
        ...
        self.assertTrue(bool(first_group.loc["BBB", "catalyst_ticker_veto"]))
        self.assertGreater(float(first_group.loc["AAA", "catalyst_overlay_score"]), float(first_group.loc["BBB", "catalyst_overlay_score"]))
    ...
    def test_publication_backfill_is_explicitly_research_only(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            for ticker in ("AAA", "BBB", "CCC"):
                events = _events(ticker, sentiment=0.6, available_at="2026-01-05T13:30:00Z").drop(columns="available_at_utc")
    ...
    def test_missing_sentiment_fails_readiness(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            event_dir = Path(directory)
            for ticker in ("AAA", "BBB", "CCC"):
                _events(ticker, sentiment=0.5, available_at="2026-01-05T13:30:00Z").drop(columns="sentiment_numeric").to_parquet(
                    event_dir / f"{ticker}_events.parquet", index=False
                )
            _, audit = build_catalyst_overlay_evidence(
                _predictions(),
                event_directories=[event_dir],
                market_context_path=None,
                config=_overlay_config(),
    ...
    def test_stale_market_context_fails_declared_coverage_boundary(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            _events("MARKET", sentiment=-0.5, available_at="2026-01-02T12:00:00Z").to_parquet(market_context, index=False)
            _, audit = build_catalyst_overlay_evidence(
                _predictions(),
                event_directories=[event_dir],
    ...
    def test_catalyst_ablation_uses_paired_scopes_and_improves_synthetic_ranking(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            event_dir = Path(directory)
            _events("AAA", sentiment=0.95, available_at="2026-01-05T13:30:00Z").to_parquet(event_dir / "AAA_events.parquet", index=False)
            ...
            _events("CCC", sentiment=0.0, available_at="2026-01-05T13:30:00Z").to_parquet(event_dir / "CCC_events.parquet", index=False)
            evidence, _ = build_catalyst_overlay_evidence(
                _predictions(),
                event_directories=[event_dir],
                market_context_path=None,
                config=_overlay_config(overlay_weight=0.5),
            )
            report, selected = evaluate_catalyst_ablation(
                evidence,
                config=CatalystAblationConfig(top_k=1, bootstrap_iterations=100, minimum_sessions=2),
            )
        self.assertEqual(set(report["scopes"]), {"walk_forward", "ticker_holdout"})
        self.assertGreater(
            report["scopes"]["walk_forward"]["catalyst_overlay"]["mean_top_k_excess_return"],
            report["scopes"]["walk_forward"]["technical_ranker"]["mean_top_k_excess_return"],
        )
        self.assertEqual(set(selected["strategy"]), {"technical_ranker", "catalyst_overlay"})


def _overlay_config(**updates: object) -> CatalystOverlayConfig:
    ...
    return CatalystOverlayConfig(**values)  # type: ignore[arg-type]
