    ...
    *,
    ...
    if (
        ...
        raise DataReadinessError(
            "S&P raw archive and event output directories must be disjoint"
        )
    ...
    try:
        ...
        raise DataReadinessError(
            f"another process is writing S&P events {output_directory}"
        ) from exc
...
def _extract_spglobal_events_locked(
    *,
    ...
    manifest: dict[str, Any] = {
        ...
        "parsed_release_count": sum(
            item["disposition"] == "parsed" for item in outcomes
        ),
        "no_effective_event_release_count": sum(
            item["disposition"] == "no_effective_event" for item in outcomes
        ),
...
def require_spglobal_event_reconstruction_ready(
    ...
    *,
    ...
    if not isinstance(authority_value, dict) or not all(
        isinstance(key, str) for key in authority_value
    ):
    ...
    if (
        ...
        != archive.authority.get("artifact_sha256")
        or authority.get("raw_manifest_sha256")
        != archive.authority.get("artifact_sha256")
        or manifest.get("raw_release_set_sha256")
        != archive.manifest.get("release_set_sha256")
    ...
    replayed_assertions, replayed_outcomes, unresolved = _parse_verified_archive(
        archive
    )
    ...
    if (
        ...
        or int(manifest.get("parsed_release_count", -1))
        != sum(item["disposition"] == "parsed" for item in replayed_outcomes)
        or int(manifest.get("no_effective_event_release_count", -1))
        != sum(
            item["disposition"] == "no_effective_event"
            for item in replayed_outcomes
        )
        or int(manifest.get("duplicate_support_count", -1))
        != _duplicate_support_count(replayed_assertions)
        ...
        supporting_sources = tuple(
            IndexChangeSource(
                ...
                source_published_date=date.fromisoformat(
                    str(source["source_published_date"])
                ),
...
def _parse_verified_archive(
    ...
    for record in sorted(archive.releases, key=lambda item: str(item["url"])):
        try:
            ...
            unresolved.append(
                {"source_url": str(record["url"]), "error": error}
            )
            outcomes.append(
                _release_outcome(
                    record,
            ...
            continue
        if any(
            change.source_published_date > change.effective_at_utc.date()
            for change in changes
        ):
            error = (
                "DataReadinessError: S&P event effective date precedes its "
