...
def deterministic_stratified_ticker_holdout(
    ...
    *,
    ...
    missing = sorted(required.difference(frame.columns))
    if missing:
        raise DataReadinessError(f"stratified ticker holdout input missing columns: {', '.join(missing)}")
    if frame.empty:
    ...
    if not failed.empty:
        detail = ", ".join(f"{row.dimension}={row.stratum}" for row in failed.itertuples(index=False))
    ...
    summary_hash = hashlib.sha256(json.dumps(summary_records, sort_keys=True, separators=(",", ":")).encode("utf-8")).hexdigest()
...
def _ticker_summaries(
    ...
    *,
    ...
    cap_available = "market_cap_bucket" in data.columns or _has_valid_one_hot(data, "market_cap_")
    liquidity_available = "liquidity_bucket" in data.columns or _has_valid_one_hot(data, "liquidity_")
...
def _balanced_assignment(
    ...
    *,
    ...
    order = {
        ticker: rank
        for rank, ticker in enumerate(sorted(tickers, key=lambda value: hashlib.sha256(f"{seed}:{value}".encode()).hexdigest()))
    }
    desired = {key: min(len(members) - 1, max(1, round(len(members) * fraction))) for key, members in groups.items()}
    ...
    for _ in range(max(1, len(groups) * 2)):
        if objective(selected)[0] == 0:
            return selected
        trials = [(selected - {removed}) | {added} for removed in sorted(selected) for added in tickers if added not in selected]
