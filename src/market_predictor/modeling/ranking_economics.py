...
def fit_disjoint_calibrator(
    ...
    *,
    ...
    if family not in probabilistic_families:
        raise DataReadinessError("only probabilistic classifier families can be calibrated")
    ...
    data = predictions[predictions["family"].astype(str).eq(family) & predictions["audit_scope"].astype(str).eq("walk_forward")].copy()
...
def build_multi_output_evidence(
    ...
    *,
    ...
    downside_family: str = "downside_classifier",
    ...
    opportunity = predictions[
        predictions["family"].astype(str).eq(opportunity_family) & predictions["audit_scope"].astype(str).eq(audit_scope)
    ].copy()
    downside = predictions[
        predictions["family"].astype(str).eq(downside_family) & predictions["audit_scope"].astype(str).eq(audit_scope)
    ].copy()
    ...
    opportunity = opportunity.rename(columns={"score": "raw_opportunity_score", "opportunity_score": "opportunity_score"})
    ...
    if downside_calibration is not None:
        ...
        downside = downside.rename(columns={"calibrated_probability": "downside_probability", "model_run_id": "downside_model_run_id"})
        downside["downside_calibrated"] = 1
    else:
        downside = downside.rename(columns={"score": "downside_probability", "model_run_id": "downside_model_run_id"})
