...
from market_predictor.universe.sp500.universe import ARCHIVE_QUERY, SP_GLOBAL_ARCHIVE_URL
...
class ArchiveCollectionConfig:
    ...
    def validate(self) -> None:
        if self.discovery_end < DISCOVERY_START:
            raise ValueError(f"discovery_end must be on or after {DISCOVERY_START.isoformat()}")
...
def collect_spglobal_archive(
    *,
    ...
    try:
        ...
        raise DataReadinessError(f"another collector is already writing S&P archive {output_directory}") from exc
...
def _collect_spglobal_archive_locked(
    *,
    ...
    if any(seed.published_date > policy.discovery_end for seed in seeds):
        raise DataReadinessError("source audit contains a frozen seed published after discovery_end")
    ...
    if not all_complete:
        return _publish_status(
            ...
            request_sha256=request_sha256,
            status="incomplete",
            stop_reason=("release_failures" if failures else "operational_batch_limit"),
    ...
    manifest: dict[str, Any] = {
        **status,
        ...
        "release_set_sha256": _json_sha256([_raw_release_identity(record) for record in raw_release_records]),
    }
...
def require_spglobal_raw_archive_complete(
    ...
    release_set_sha256 = _json_sha256([_raw_release_identity(record) for record in releases])
    if (
        ...
        or manifest.get("source_audit_sha256") != authority.get("source_audit_sha256")
        ...
        raise DataReadinessError("S&P raw archive manifest lineage or release counts are invalid")
    ...
    if (
        ...
        raise DataReadinessError("S&P raw archive request identity is invalid")
    try:
        ...
        raise DataReadinessError("S&P raw archive request discovery boundary is invalid") from exc
    if discovery_start != DISCOVERY_START or discovery_end < discovery_start:
        raise DataReadinessError("S&P raw archive request discovery boundary is invalid")
    discovery = _load_json(archive_directory / "_discovery.json")
    request_seed_urls = request.get("seed_urls")
    if not isinstance(request_seed_urls, list) or not all(isinstance(url, str) for url in request_seed_urls):
        raise DataReadinessError("S&P raw archive request seed set is invalid")
    seeds = [_announcement_from_record(record) for record in _records(discovery, "announcements") if record.get("origin") == "seed"]
    if len(seeds) != EXPECTED_SEED_URLS or [seed.url for seed in seeds] != request_seed_urls:
        raise DataReadinessError("S&P raw archive discovery does not retain the frozen seed set")
    ...
    if (
        ...
        or [_raw_release_record(record) for record in replayed_releases] != releases
        or manifest.get("discovery_sha256") != _file_sha256(archive_directory / "_discovery.json")
    ):
        raise DataReadinessError("S&P raw archive manifest does not equal the retained unit replay")
...
def read_verified_spglobal_release_html(
    ...
    if not any((item.get("url"), item.get("sha256"), item.get("unit_id")) == identity for item in archive.releases):
        raise DataReadinessError("S&P release is not a member of the verified raw archive")
    return _decode_html(
...
def _replay_complete_archive(
    root: Path,
    *,
    seeds: list[SpGlobalAnnouncement],
    request_sha256: str,
    discovery_end: date,
    ...
    announcements = [_announcement_from_record(item) for item in _records(discovery, "announcements")]
    expected_release_units = {f"{_release_unit_id(item.url)}.json" for item in announcements}
    release_directory = root / "units" / "releases"
    observed_release_units = {path.name for path in release_directory.glob("*.json")} if release_directory.is_dir() else set()
    if observed_release_units != expected_release_units:
        raise DataReadinessError("S&P official archive release sidecars do not match discovery")
    ...
    for announcement in sorted(announcements, key=lambda item: item.url):
        ...
        if record is None:
            raise DataReadinessError(f"S&P official archive release is missing: {announcement.url}")
...
def _discover_or_resume(
    *,
    ...
    discovered: dict[str, SpGlobalAnnouncement] = {}
    ...
    for page_number in range(config.maximum_pages):
        ...
        page_announcements, dated_urls = parse_spglobal_archive_search_page(
            decode_spglobal_http_entity(body, str(record.get("content_encoding") or "")),
    ...
    if not boundary_reached or not upper_boundary_crossed:
        _publish_status(
            ...
            request_sha256=request_sha256,
            status="incomplete",
            stop_reason=("discovery_boundary_not_reached" if not boundary_reached else "discovery_upper_boundary_not_crossed"),
            discovery_complete=False,
            requested_releases=0,
            completed_releases=0,
            resumed_releases=0,
        ...
        return (
            None,
            network_units,
            "discovery_boundary_not_reached" if not boundary_reached else "discovery_upper_boundary_not_crossed",
        )
...
def _collect_releases(
    *,
    ...
    pending: list[SpGlobalAnnouncement] = []
    resumed = 0
    ...
    def collect_one(announcement: SpGlobalAnnouncement) -> dict[str, Any]:
    ...
    futures: dict[Future[dict[str, Any]], SpGlobalAnnouncement] = {}
    ...
    for observed_url in (
        ...
        *response.redirect_chain,
        ...
        if not _same_search_identity(str(observed_url), identity):
            raise DataReadinessError(f"official archive search redirect changed the query identity: {identity} -> {observed_url}")
...
def _persist_release(
    root: Path,
    *,
    announcement: SpGlobalAnnouncement,
    request_sha256: str,
    response: BytesResponse,
) -> dict[str, Any]:
    metadata = _validate_response(response, expected_release=True)
    if _canonical_release_path(response.final_url) != _canonical_release_path(announcement.url):
        raise DataReadinessError(f"official release redirect changed the release identity: {announcement.url} -> {response.final_url}")
    body = response.body
...
def _validate_response(response: BytesResponse, *, expected_release: bool) -> dict[str, Any]:
    ...
    if response.body_representation != "http_entity_encoded":
        raise DataReadinessError("official response body representation must be http_entity_encoded")
    ...
    if isinstance(retrieved, datetime) and retrieved.tzinfo is None:
        raise DataReadinessError("official response retrieval timestamp must be timezone-aware")
...
def parse_spglobal_archive_search_page(
    body: bytes,
    *,
    base_url: str,
    content_type: str,
) -> tuple[list[SpGlobalAnnouncement], list[tuple[date, str]]]:
    results, dated_urls = parse_spglobal_archive_search_inventory(
        body,
        base_url=base_url,
    ...
    return (
        [
            ...
            for item in results
            if _is_membership_title(item.title)
        ],
        dated_urls,
    )

def parse_spglobal_archive_search_inventory(
    body: bytes,
    *,
    base_url: str,
    content_type: str,
) -> tuple[list[SpGlobalAnnouncement], list[tuple[date, str]]]:
    """Return every dated official result so observed frontiers cannot hide titles."""

    html = _decode_html(body, content_type or "text/html; charset=utf-8")
    soup = BeautifulSoup(html, "html.parser")
    results: dict[str, SpGlobalAnnouncement] = {}
    ...
    for anchor in soup.find_all("a", href=True):
        ...
        if previous is not None and previous != published:
            raise DataReadinessError(f"official archive URL has conflicting publication dates: {url}")
        if previous is None:
        ...
        results[url] = SpGlobalAnnouncement(published, title, url, "observation")
    return (
        sorted(
            results.values(),
...
def _validate_search_page_coverage(
    *,
    ...
    if any(left < right for left, right in zip(dates, dates[1:], strict=False)):
        raise DataReadinessError("official archive search page is not newest-to-oldest")
    overlap = seen_urls.intersection(urls)
    if page_number == 0:
        if previous_last_url is not None or overlap:
            raise DataReadinessError("official archive first page has invalid pagination state")
    elif previous_last_url is None or dated_urls[0][1] != previous_last_url or overlap != {previous_last_url}:
        raise DataReadinessError("official archive pagination lacks the required one-result overlap")
    newest = max(dates)
    oldest = min(dates)
    try:
        observed_at = datetime.fromisoformat(retrieved_at_utc)
    except ValueError as exc:
        raise DataReadinessError("official archive search retrieval timestamp is invalid") from exc
    if observed_at.tzinfo is None:
        raise DataReadinessError("official archive search retrieval timestamp is not timezone-aware")
    observed_through_cutoff = observed_at.astimezone(_SOURCE_TIMEZONE).date() > discovery_end
    if page_number == 0 and newest < discovery_end and not observed_through_cutoff:
        raise DataReadinessError("official archive first page was observed before the frozen upper boundary")
    seen_urls.update(urls)
    return dated_urls[-1][1], (
        upper_boundary_crossed
        or newest >= discovery_end >= oldest
        or (page_number == 0 and newest < discovery_end and observed_through_cutoff)
    )

def _load_seed_announcements(path: Path) -> list[SpGlobalAnnouncement]:
    ...
    if not isinstance(source_manifest, Mapping) or source_manifest.get("schema") != SOURCE_MANIFEST_SCHEMA:
    ...
    by_url: dict[str, SpGlobalAnnouncement] = {}
    for raw in raw_sources:
        ...
        by_url[url] = SpGlobalAnnouncement(published, title, url, "seed")
    if len(by_url) != EXPECTED_SEED_URLS:
        raise DataReadinessError(f"frozen source audit must contain exactly {EXPECTED_SEED_URLS} distinct URLs, got {len(by_url)}")
    audit_urls = audit.get("source_urls")
    if isinstance(audit_urls, list) and not {str(item) for item in audit_urls}.issubset(by_url):
        raise DataReadinessError("source audit URL inventory contains URLs outside source_manifest.sources")
...
def _load_unit(
    ...
    *,
    ...
    record_without_hash = {key: value for key, value in record.items() if key != "unit_sha256"}
...
def _validate_stored_unit_metadata(
    ...
    *,
    ...
    try:
        ...
        raise DataReadinessError("S&P official archive retrieval timestamp is invalid") from exc
    if retrieved.tzinfo is None:
        raise DataReadinessError("S&P official archive retrieval timestamp is not timezone-aware")
    ...
    if expected_release:
        if _is_generic_landing_url(str(record["final_url"])):
            raise DataReadinessError("S&P official release resume points to a generic landing page")
    elif any(not _same_search_identity(url, str(record["identity"])) for url in urls):
        raise DataReadinessError("S&P official archive search resume changed the query identity")
    if record["body_representation"] != "http_entity_encoded":
        raise DataReadinessError("S&P official archive body representation is invalid")
...
def _validate_release_record(
    ...
    if (
        ...
        or _canonical_release_path(str(record.get("requested_url", ""))) != _canonical_release_path(announcement.url)
        or _canonical_release_path(str(record.get("final_url", ""))) != _canonical_release_path(announcement.url)
    ):
...
def _raw_release_record(record: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if missing:
        raise DataReadinessError(f"S&P raw release metadata is incomplete: {missing}")
...
def _validate_discovery(
    ...
    *,
    ...
    if not page_records:
        raise DataReadinessError("S&P official archive discovery has no search pages")
    discovered: dict[str, SpGlobalAnnouncement] = {}
    boundary_reached = False
    previous_last_url: str | None = None
    upper_boundary_crossed = False
    ...
    for page_number, page_record in enumerate(page_records):
        ...
        expected_unit_id = f"search-{page_number:04d}-{hashlib.sha256(expected_identity.encode()).hexdigest()[:12]}"
        if (
            ...
            raise DataReadinessError("S&P official archive discovery pages are not contiguous from page zero")
        loaded = _load_unit(
            root,
            kind="search_page",
        ...
        if loaded != page_record:
            raise DataReadinessError("S&P official archive discovery differs from search-page sidecar")
        page_announcements, dated_urls = parse_spglobal_archive_search_page(
            decode_spglobal_http_entity(
        ...
        if max(published for published, _ in dated_urls) < DISCOVERY_START:
            if page_number != len(page_records) - 1:
                raise DataReadinessError("S&P official archive discovery continued after its lower boundary")
            boundary_reached = True
    expected = [item.to_record() for item in _union_announcements(seeds, discovered)]
    if not boundary_reached or not upper_boundary_crossed or announcements != expected:
        raise DataReadinessError("S&P official archive discovery does not replay from retained search pages")
    if int(discovery.get("discovered_url_count", -1)) != len(discovered) or int(discovery.get("release_url_count", -1)) != len(expected):
    ...
    with _OBJECT_WRITE_LOCK:
        ...
        if path.exists():
            if _file_sha256(path) != digest or path.read_bytes() != body:
                raise DataReadinessError(f"content-addressed object is corrupt: {path}")
            return path
        ...
        try:
            ...
            if _file_sha256(temporary) != digest:
                raise DataReadinessError(f"stored response bytes failed post-write verification: {path}")
...
def _announcement_from_record(record: Mapping[str, Any]) -> SpGlobalAnnouncement:
    try:
        published = date.fromisoformat(str(record["published_date"]))
        url = str(record["url"])
        _require_spglobal_url(url)
        return SpGlobalAnnouncement(published, str(record.get("title", "")), url, str(record.get("origin", "")))
    except (KeyError, ValueError) as exc:
        raise DataReadinessError("invalid S&P official archive announcement record") from exc

def _union_announcements(
    seeds: list[SpGlobalAnnouncement],
    discovered: Mapping[str, SpGlobalAnnouncement],
    ...
    for url, announcement in discovered.items():
        ...
        if seed_item is None:
            ...
            raise DataReadinessError(f"conflicting publication dates for official release: {url}")
        elif not seed_item.title and announcement.title:
            union[url] = SpGlobalAnnouncement(
                published_date=seed_item.published_date,
                title=announcement.title,
                url=url,
                origin="seed_and_discovery",
            )
    return sorted(union.values(), key=lambda item: (item.published_date, item.url))
...
def _same_search_identity(actual_url: str, expected_url: str) -> bool:
    ...
    return actual.path.rstrip("/") == expected.path.rstrip("/") and dict(parse_qsl(actual.query, keep_blank_values=True)) == dict(
...
def decode_spglobal_http_entity(
    body: bytes,
    ...
    *,
    ...
    decoded = body
    encodings = [item.strip().lower() for item in content_encoding.split(",") if item.strip() and item.strip().lower() != "identity"]
    if not encodings and len(body) > maximum_decoded_bytes:
        raise DataReadinessError("official response decoded body exceeds the configured limit")
    try:
        for encoding in reversed(encodings):
            if encoding in {"gzip", "x-gzip"}:
                ...
                raise DataReadinessError(f"unsupported official response Content-Encoding: {encoding}")
    except zlib.error as exc:
        raise DataReadinessError("official response Content-Encoding cannot be decoded") from exc
    return decoded
...
def _decompress_bounded(
    ...
    *,
    ...
    if not decompressor.eof:
        raise DataReadinessError("official response Content-Encoding is truncated")
    return b"".join(output)

def decode_spglobal_html(body: bytes, content_type: str) -> str:
    """Decode a bounded official HTML entity using its declared charset."""

    return _decode_html(body, content_type)

_decode_http_entity = decode_spglobal_http_entity
