"""Intraday-research acquisition planning, retired with the intraday package.

The retained plan helpers live in `market_predictor.collection.alpaca_bars.plan`.
"""
...
from market_predictor.collection.alpaca_bars.contracts import REGULAR_BAR_HISTORY_PLAN_SCHEMA, RegularBarHistoryConfig
...
# Explicit re-export: bar_dataset.py imports it from here, and its source bytes are the recorded A4.3
# transformation identity, so it stays byte-identical until (d) deletes both.
from market_predictor.evidence.hashing import json_sha256 as json_sha256
...
# Every plan layer the intraday research wrote, including the two the retained collectors still write.
...
def build_intraday_history_plan(
    *,
    ...
    config: RegularBarHistoryConfig,
) -> dict[str, Any]:
