...
def phase_economics(
    ...
    *,
    ...
    for phase in range(phase_count):
        ...
        selected = selected_all[selected_all["decision_group_id"].isin(phase_groups)].copy()
        records.append(
            _economic_record(
                selected,
...
def regime_audit(
    ...
    *,
    ...
    for regime in audit_regimes:
        ...
        record = conservative_economics(populated_economics).iloc[0] if not populated_economics.empty else pd.Series(dtype=object)
        ...
        details.append(
            {
                ...
                "avg_trade_return_ci_low": finite_or_none(record.get("avg_trade_return_ci_low")),
                "avg_excess_return_vs_spy_ci_low": finite_or_none(record.get("avg_excess_return_vs_spy_ci_low")),
...
def _economic_record(
    ...
    *,
    ...
    if cost is not None and gross_column in selected.columns:
        ...
        excess = {
            benchmark: (
                net
                - pd.to_numeric(
                    selected[f"path_{benchmark}_return_{horizon_minutes}m"],
                    errors="coerce",
                )
            ).dropna()
    ...
    group_returns = work.groupby(["session_date_et", "decision_group_id"], sort=False)["_net"].mean().dropna()
