...
def build_cross_sectional_features(
    ...
    *,
    ...
    technical = build_ticker_features(bars, source_availability=source_availability)
    return finalize_cross_sectional_features(
        technical,
        benchmarks,
...
def finalize_cross_sectional_features(
    ...
    *,
    ...
    data["feature_schema_version"] = CROSS_SECTIONAL_FEATURE_SCHEMA_VERSION
    return data.sort_values(["decision_time_utc", "ticker"]).reset_index(drop=True)
...
def core_feature_columns() -> tuple[str, ...]:
    ...
    cross_sectional = [name for feature in CROSS_SECTIONAL_BASE_FEATURES for name in (f"xs_rank_{feature}", f"xs_robust_z_{feature}")]
...
def _add_technical_features(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    data["atr_14"] = true_range.groupby(data["ticker"], sort=False).transform(lambda values: values.ewm(alpha=1 / 14, adjust=False).mean())
