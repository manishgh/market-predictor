...
def expected_calibration_error(
    ...
    *,
    ...
    p = pd.to_numeric(probability, errors="coerce").clip(0, 1).to_numpy(float)
    ...
    *,
    ...
    for phase in range(phase_count):
        ...
        candidates = predictions[
            predictions["decision_group_id"].isin(phase_groups) & predictions["intraday_downside_probability"].le(downside_ceiling)
        ].copy()
        selected = (
        ...
        selected = (
            selected.sort_values(
                ["session_date_et", "decision_time_utc", "intraday_opportunity_probability"],
...
def _economic_record(
    ...
    *,
    ...
    return_column = net_return_column(horizon_minutes)
    returns = pd.to_numeric(selected.get(return_column), errors="coerce").dropna()
    excess = {
        benchmark: pd.to_numeric(
            selected.get(excess_return_column(horizon_minutes, benchmark)),
            errors="coerce",
        ).dropna()
        for benchmark in ("spy", "qqq", "sector")
    }
    group_returns = (
        selected.assign(_return=pd.to_numeric(selected.get(return_column), errors="coerce"))
        .groupby(["session_date_et", "decision_group_id"], sort=False)["_return"]
        .mean()
        .dropna()
    )
    session_returns = group_returns.groupby(level="session_date_et").sum()
