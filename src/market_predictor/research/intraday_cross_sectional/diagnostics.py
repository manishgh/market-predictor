...
from market_predictor.research.intraday_cross_sectional.partitions import assert_development_only
...
def build_failure_attribution(
    ...
    *,
    ...
    if missing_predictions:
        raise DataReadinessError(f"failure-attribution predictions missing columns: {', '.join(missing_predictions)}")
    missing_development = sorted(set(FAILURE_ATTRIBUTION_DATASET_COLUMNS).difference(development.columns))
    if missing_development:
        raise DataReadinessError(f"failure-attribution dataset missing columns: {', '.join(missing_development)}")
    ...
    if scopes != required_scopes:
        raise DataReadinessError(f"failure attribution requires exactly {sorted(required_scopes)}; received {sorted(scopes)}")
    ...
    report: dict[str, Any] = {
        ...
        "strata_summary": {
            ...
            "positive_reliable_rows": int((strata["meets_minimum_evidence"] & strata["selected_mean_excess_60m"].gt(0)).sum()),
...
def _validate_joined_targets(joined: pd.DataFrame) -> None:
    ...
    if maximum_difference > 1e-12:
        raise DataReadinessError(f"prediction and frozen dataset ranking targets differ by as much as {maximum_difference}")
...
def _scope_report(
    ...
    *,
    ...
    horizon_reports = {outcome: _outcome_report(scope_data, selected, outcome=outcome, config=config) for outcome in OUTCOME_COLUMNS}
    ...
    score_percentile = scope_data.groupby("decision_group_id", observed=True)["score"].rank(method="first", pct=True)
    ...
    return {
        ...
        "mean_cross_section_rank_correlation_60m": (float(np.mean(rank_correlations)) if rank_correlations else None),
...
def _outcome_report(
    ...
    *,
    ...
    oracle_groups = (
        population.groupby("decision_group_id", observed=True)[outcome].nlargest(config.top_k).groupby(level=0).mean().rename("oracle")
    )
                ...
                rows.append(
                    {
                        ...
                        "selected_mean_return_60m": float(pd.to_numeric(selected_stratum["net_return_60m"], errors="coerce").mean()),
                        "selected_mean_mfe_60m": float(pd.to_numeric(selected_stratum["mfe_60m"], errors="coerce").mean()),
                        "selected_mean_mae_60m": float(pd.to_numeric(selected_stratum["mae_60m"], errors="coerce").mean()),
                        "meets_minimum_evidence": (
                            len(selected_stratum) >= config.minimum_stratum_rows and sessions >= config.minimum_stratum_sessions
                        ),
                    }
                )
