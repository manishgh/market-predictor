...
CROSS_SECTIONAL_LABEL_SCHEMA_VERSION = "ml_v3.labels.v2"


class OpportunityLabelConfig(FrozenContract):
    ...
    schema_version: str = CROSS_SECTIONAL_LABEL_SCHEMA_VERSION
...
def build_opportunity_labels(
    ...
    *,
    config: OpportunityLabelConfig = OpportunityLabelConfig(),
    ...
    """Build cost-adjusted point-in-time labels from next-open entries."""
...
def _label_session(
    ...
    regular_indices = np.flatnonzero((minute_et >= config.decision_start_minute_et) & (minute_et < config.decision_end_minute_et))
    ...
    if "feature_schema_version" not in output.columns:
        output["feature_schema_version"] = CROSS_SECTIONAL_SCHEMA_VERSION
