...
def build_o1_overlay_evidence(
    ...
    *,
    ...
    data["o1_global_veto"] = data["global_event_count_2h"].ge(config.minimum_global_veto_events) & data["global_sentiment_mean_2h"].le(
        config.global_veto_sentiment
    )
...
def evaluate_o1_ablation(
    ...
    *,
    ...
    passed = all(
        reports[scope]["o1"]["mean_top_k_excess_return"] > 0 and reports[scope]["paired_excess_return_delta_interval"]["low"] >= 0
...
def _evaluate_scope(
    ...
    *,
    ...
    for group_id, group in data.groupby("decision_group_id", sort=False, observed=True):
        ...
        if int(valid.sum()) < 2:
            continue
        baseline_ndcg.append(float(ndcg_score([grade[valid].to_numpy()], [group.loc[valid, "r1_rank_percentile"].to_numpy()], k=k)))
        ...
        paired.append(
            {
                ...
                "o1_excess_return": (float(pd.to_numeric(overlay["ranking_target"], errors="coerce").mean()) if not overlay.empty else 0.0),
            }
        )
    if not paired:
...
def _prepare_event_frame(
    ...
    *,
    ...
    clean = clean[clean["available_at_utc"].between(coverage_start - max(O1_WINDOWS.values()), coverage_end, inclusive="both")].copy()
