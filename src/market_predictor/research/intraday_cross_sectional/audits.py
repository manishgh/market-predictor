...
from market_predictor.core.contracts import UniverseMembership
...
def audit_benchmarks(decisions: pd.DataFrame) -> tuple[AuditCheck, ...]:
    ...
    missing_value = int(decisions[["primary_benchmark", "benchmark_close"]].replace(r"^\s*$", pd.NA, regex=True).isna().any(axis=1).sum())
