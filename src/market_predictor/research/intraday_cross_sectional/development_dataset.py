...
def build_monthly_development_dataset(
    *,
    ...
    """Build memory-bounded labels through per-ticker and monthly stages."""
    ...
    label_config = OpportunityLabelConfig(
    ...
    for month_directory in sorted(technical_directory.glob("month_*")):
        ...
        labels = build_opportunity_labels(
            technical,
            benchmarks,
            config=label_config,
        ...
        features = finalize_cross_sectional_features(
            labels,
            benchmarks,
    ...
    if not month_results or sum(int(item["label_rows"]) for item in month_results) == 0:
        raise DataReadinessError("no monthly cross-sectional development labels were produced")
    report["months"] = month_results
    report["summary"] = {
...
def _build_all_ticker_shards(
    *,
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        futures = {executor.submit(_build_ticker_shards, path, memberships, availability, technical_directory): path for path in bar_files}
...
def _build_ticker_shards(
    ...
    technical = build_ticker_features(
        bars,
        source_availability=ticker_availability,
    )
...
def _restrict_to_market_session_grid(
    ...
    *,
    ...
    expected_parts = [pd.date_range(row.open, row.close, freq=f"{interval_minutes}min", inclusive="left") for row in schedule.itertuples()]
    ...
    market = benchmark_identity[benchmark_identity["ticker"].isin(required) & benchmark_identity["timestamp"].isin(expected_grid)]
    ...
    if not missing_grid.empty:
        raise DataReadinessError(
            f"Required benchmark market grid is incomplete at {missing_grid.index[0]}: symbols={int(missing_grid.iloc[0])}/{len(required)}"
        )
