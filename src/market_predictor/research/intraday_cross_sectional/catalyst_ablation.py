...
class CatalystOverlayConfig(FrozenContract):
    ...
    def validate_timestamp(cls, value: datetime) -> datetime:
        ...
        if timestamp.tzinfo is None:
            raise ValueError("catalyst coverage timestamps must be timezone-aware")
        return cast(datetime, timestamp.tz_convert("UTC").to_pydatetime())

    @model_validator(mode="after")
    def validate_window(self) -> CatalystOverlayConfig:
        if self.coverage_start_utc >= self.coverage_end_utc:
            raise ValueError("catalyst coverage start must precede coverage end")
        return self


class CatalystAblationConfig(FrozenContract):
...
def build_catalyst_overlay_evidence(
    predictions: pd.DataFrame,
    *,
    event_directories: list[Path],
    market_context_path: Path | None,
    config: CatalystOverlayConfig,
    ...
    if missing:
        raise DataReadinessError(f"catalyst prediction evidence missing columns: {', '.join(missing)}")
    ...
    if not directories or missing_directories:
        raise DataReadinessError(f"catalyst event directories are missing: {missing_directories or 'none provided'}")

    data = predictions[predictions["family"].astype(str).eq(config.base_family)].copy()
    if data.empty:
        raise DataReadinessError(f"catalyst ablation requires {config.base_family} OOF predictions")
    data["ticker"] = data["ticker"].astype(str).str.upper().str.strip()
    data["decision_time_utc"] = _utc_series(data["decision_time_utc"])
    if bool(data["decision_time_utc"].isna().any()):
        raise DataReadinessError("catalyst predictions contain invalid decision timestamps")
    run_ids = set(data["model_run_id"].astype(str))
    if len(run_ids) != 1:
        raise DataReadinessError("catalyst ablation requires exactly one base model_run_id")
    identity = ["ticker", "decision_time_utc", "decision_group_id", "audit_scope"]
    if bool(data.duplicated(identity).any()):
        raise DataReadinessError("catalyst predictions contain duplicate decision identities")
    ...
    if data.empty:
        raise DataReadinessError("catalyst ablation has no prediction rows inside declared source coverage")
    ...
    if market_context_path is not None:
        if not market_context_path.exists():
            raise DataReadinessError(f"catalyst market context file is missing: {market_context_path}")
    ...
    data["technical_rank_percentile"] = data.groupby(["audit_scope", "decision_group_id"], observed=True)["score"].rank(
        method="average",
        pct=True,
    )
    ticker_signal = 0.65 * data["ticker_signal_2h"] + 0.35 * data["ticker_signal_1d"]
    data["catalyst_ticker_signal"] = ticker_signal.clip(-1, 1)
    data["catalyst_ticker_veto"] = data["ticker_negative_material_count_1d"].gt(0)
    data["catalyst_global_veto"] = data["global_event_count_2h"].ge(config.minimum_global_veto_events) & data[
        "global_sentiment_mean_2h"
    ].le(config.global_veto_sentiment)
    data["catalyst_eligible"] = ~(data["catalyst_ticker_veto"] | data["catalyst_global_veto"])
    data["catalyst_overlay_score"] = (
        data["technical_rank_percentile"]
        + config.overlay_weight * data["catalyst_ticker_signal"]
        - config.ticker_veto_penalty * data["catalyst_ticker_veto"].astype(float)
    )
    data["catalyst_status"] = np.select(
        [
            data["catalyst_global_veto"],
            data["catalyst_ticker_veto"],
            data["catalyst_ticker_signal"].ge(0.15),
            data["catalyst_ticker_signal"].le(-0.15),
    ...
    future_matches = int(
        (data["ticker_latest_event_at_utc"].notna() & (data["ticker_latest_event_at_utc"] > data["decision_time_utc"])).sum()
    )
    readiness_failures: list[str] = []
    if ticker_file_coverage < config.minimum_ticker_file_coverage:
        readiness_failures.append(f"ticker event-file coverage {ticker_file_coverage:.4f} < {config.minimum_ticker_file_coverage:.4f}")
    if sentiment_coverage < config.minimum_sentiment_coverage:
        readiness_failures.append(f"relevant-event sentiment coverage {sentiment_coverage:.4f} < {config.minimum_sentiment_coverage:.4f}")
...
def evaluate_catalyst_ablation(
    evidence: pd.DataFrame,
    *,
    config: CatalystAblationConfig = CatalystAblationConfig(),
    ...
    required = {
        ...
        "path_realized_return_net",
        "independent_event_id",
        "technical_rank_percentile",
        "catalyst_overlay_score",
        "catalyst_eligible",
        "catalyst_status",
    }
    missing = sorted(required.difference(evidence.columns))
    if missing:
        raise DataReadinessError(f"catalyst ablation evidence missing columns: {', '.join(missing)}")
    ...
    if missing_scopes:
        raise DataReadinessError(f"catalyst ablation missing audit scopes: {', '.join(missing_scopes)}")
    passed = all(
        reports[scope]["catalyst_overlay"]["mean_top_k_excess_return"] > 0
    ...
    report = {
        ...
        "promotion_blocker": ("catalyst ablation requires strict catalyst provenance, a valid downside model, and frozen gates"),
    }
...
def _evaluate_scope(
    ...
    *,
    ...
    for group_id, group in data.groupby("decision_group_id", sort=False, observed=True):
        ...
        if int(valid.sum()) < 2:
            continue
        baseline_ndcg.append(float(ndcg_score([grade[valid].to_numpy()], [group.loc[valid, "technical_rank_percentile"].to_numpy()], k=k)))
        overlay_ndcg.append(float(ndcg_score([grade[valid].to_numpy()], [group.loc[valid, "catalyst_overlay_score"].to_numpy()], k=k)))
        baseline = group.nlargest(k, "technical_rank_percentile").copy()
        eligible = group[group["catalyst_eligible"].astype(bool)]
        overlay = eligible.nlargest(min(k, len(eligible)), "catalyst_overlay_score").copy() if not eligible.empty else eligible.copy()
        baseline["strategy"] = "technical_ranker"
        overlay["strategy"] = "catalyst_overlay"
        ...
        paired.append(
            {
                ...
                "technical_excess_return": float(pd.to_numeric(baseline["ranking_target"], errors="coerce").mean()),
                "catalyst_overlay_excess_return": (
                    float(pd.to_numeric(overlay["ranking_target"], errors="coerce").mean()) if not overlay.empty else 0.0
                ),
            }
        )
    if not paired:
        raise DataReadinessError(f"catalyst {scope} scope contains no auditable ranking groups")
    ...
    paired_frame["delta"] = paired_frame["catalyst_overlay_excess_return"] - paired_frame["technical_excess_return"]
    ...
    report = {
        ...
        "technical_ranker": _selection_summary(baseline_selected, baseline_ndcg),
        "catalyst_overlay": _selection_summary(overlay_selected, overlay_ndcg),
...
def _selection_summary(selected: pd.DataFrame, ndcg_values: list[float]) -> dict[str, Any]:
    ...
    catalyst_rate = float(selected["catalyst_status"].astype(str).isin({"confirmed", "mixed", "conflicting"}).mean())
    return {
    ...
    *,
    ...
    if config.availability_policy == "strict_ingestion":
        ...
        if availability_column is None:
            raise DataReadinessError("strict catalyst input requires available_at_utc or ingested_at_utc")
    ...
    clean = clean[
        clean["available_at_utc"].between(
            coverage_start - max(CATALYST_EVENT_WINDOWS.values()),
            coverage_end,
            inclusive="both",
        )
    ].copy()
...
def _aggregate_event_windows(decisions: pd.DataFrame, events: pd.DataFrame, *, prefix: str) -> pd.DataFrame:
    ...
    for window in CATALYST_EVENT_WINDOWS:
    ...
    latest = end - 1
    has_latest = latest >= 0
    latest_values = np.full(len(output), np.datetime64("NaT", "ns"), dtype="datetime64[ns]")
    ...
    for name, window in CATALYST_EVENT_WINDOWS.items():
...
def _event_id(row: pd.Series) -> str:
    payload = "|".join(str(row.get(column, "")) for column in ("ticker", "published_at_utc", "source", "title", "url"))
...
def _session_bootstrap_interval(
    ...
    *,
    ...
    if len(sessions) < 2:
        raise DataReadinessError("catalyst session bootstrap requires at least two sessions")
    values = {
