...
from market_predictor.intraday.features.cross_sectional import (
    CROSS_SECTIONAL_FEATURE_SCHEMA_VERSION,
...
CROSS_SECTIONAL_MODEL_SCHEMA_VERSION = "ml_v3.model.v1"

TRAINING_REQUIRED_COLUMNS = frozenset(
    {
        "ticker",
        "decision_time_utc",
...
PREDICTION_AUDIT_COLUMNS = frozenset(
    {
...
class CrossSectionalTrainingConfig(FrozenContract):
...
def training_input_columns() -> list[str]:
    """Return the bounded columns required by cross-sectional training."""
    return sorted(TRAINING_REQUIRED_COLUMNS | PREDICTION_AUDIT_COLUMNS | set(core_feature_columns()))


class DeterministicTechnicalRanker:
    """Deterministic non-ML floor using point-in-time technical ranks."""
...
def train_cross_sectional_model_suite(
    dataset: pd.DataFrame,
    output_dir: Path,
    *,
    config: CrossSectionalTrainingConfig = CrossSectionalTrainingConfig(),
...
def _train_family(
    ...
    *,
    ...
    payload = {
        "schema_version": CROSS_SECTIONAL_MODEL_SCHEMA_VERSION,
        "family": family,
        "model": final_model,
        "features": features,
        "target_col": target_column,
        "score_semantics": ("downside_probability" if family == "downside_classifier" else "opportunity_score"),
        "feature_schema_version": CROSS_SECTIONAL_FEATURE_SCHEMA_VERSION,
        "feature_schema_hash": feature_schema_hash(features),
        "run_id": run_id,
    ...
    manifest = write_model_manifest(
        ...
        model_type=f"intraday_cross_sectional_{family}",
        schema_version=CROSS_SECTIONAL_MODEL_SCHEMA_VERSION,
        target_col=target_column,
        features=features,
        ...
        validation_split="session_grouped_purged_walk_forward",
        extra={
            "run_id": run_id,
            "family": family,
...
def _new_model(family: ModelFamily, config: CrossSectionalTrainingConfig) -> Any:
    if family == "technical_ranker":
        return DeterministicTechnicalRanker()
    if family == "logistic_classifier":
        return Pipeline(
            [
    ...
    if family in {"gradient_boosting_classifier", "downside_classifier"}:
        return Pipeline(
            [
                ("imputer", SimpleImputer(strategy="median")),
                (
    ...
    try:
        ...
        raise DataReadinessError("xgboost ranker requires the optional dependency: python -m pip install -e '.[ranking]'") from exc
...
def _fit(
    ...
    *,
    ...
    if family == "xgboost_ranker":
        ...
        _assert_memory_budget(config, "xgboost ranker compact matrix")
        try:
            model.fit(
                matrix,
...
def _prediction_frame(
    ...
    *,
    ...
    output["score"] = score
    output["opportunity_score"] = 1.0 - score if family == "downside_classifier" else score
    return output
...
def _prepare_training_data(dataset: pd.DataFrame) -> pd.DataFrame:
    missing = sorted(TRAINING_REQUIRED_COLUMNS.difference(dataset.columns))
    if missing:
        raise DataReadinessError(f"cross-sectional training dataset missing columns: {', '.join(missing)}")
    ...
    if bool(data[time_columns].isna().any(axis=None)):
        raise DataReadinessError("cross-sectional training contains invalid decision, availability, or entry timestamps")
    if bool((data["feature_available_at_utc"] > data["decision_time_utc"]).any()):
        raise DataReadinessError("cross-sectional training contains features unavailable at decision time")
    if bool((data["entry_time_utc"] <= data["decision_time_utc"]).any()):
        raise DataReadinessError("cross-sectional training entry must follow the completed decision bar")
    if bool((data["primary_exit_time_utc"] < data["entry_time_utc"]).any()):
        raise DataReadinessError("cross-sectional primary exit cannot precede entry")
    ...
    if bool(pd.Series(entry_session).ne(declared_session).any()):
        raise DataReadinessError("cross-sectional training labels cannot cross Eastern sessions")
    if bool(pd.Series(exit_session).ne(declared_session).any()):
        raise DataReadinessError("cross-sectional primary exits cannot cross Eastern sessions")
    assert_development_only(
        data,
    ...
    if schemas != {CROSS_SECTIONAL_FEATURE_SCHEMA_VERSION}:
        raise DataReadinessError(f"unexpected cross-sectional feature schemas: {sorted(schemas)}")
    ...
    if len(label_schemas) != 1 or len(label_configs) != 1 or len(label_hashes) != 1:
        raise DataReadinessError("cross-sectional training must contain one frozen label schema and configuration")
    ...
    if bool(data.duplicated(["decision_group_id", "ticker"]).any()):
        raise DataReadinessError("cross-sectional training contains duplicate ticker/query rows")
    ...
    if bool(feed.ne("sip").any()):
        raise DataReadinessError("cross-sectional volume-sensitive training requires SIP price_feed provenance")
    if bool(data["universe_snapshot_id"].fillna("").astype(str).str.strip().eq("").any()):
        raise DataReadinessError("cross-sectional training requires universe_snapshot_id on every row")
...
def _target_column(family: ModelFamily) -> str:
    if family in {"technical_ranker", "xgboost_ranker"}:
        return "ranking_grade"
    if family == "downside_classifier":
...
def _memory_guard_callback(
    xgboost: Any,
    config: CrossSectionalTrainingConfig,
) -> Any:
    def after_iteration(self: Any, model: Any, epoch: int, evals_log: dict[str, Any]) -> bool:
        _assert_memory_budget(config, f"xgboost ranker iteration {epoch}")
        return False
