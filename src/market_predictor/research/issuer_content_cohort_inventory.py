...
_ATTRIBUTED = ("bridged", "identity_equal")
...
_SUMMARY_COLUMNS = ("archive", "chunk_id", "event_id", "query_security_id", "cohort_security_id", "query_identity_resolution",
                    "source_family", "provider_story_id", "inventory_status", "content_category", "published_at_utc",
                    "publication_year_new_york")
...
def _pin(root: Path, record: Any, pins: dict[str, str]) -> Path:
    _require(isinstance(record, dict) and set(record) == {"path", "sha256"}, "inventory config requires path/hash pins")
    path = inside(root, record["path"])
    _require(file_sha256(path) == record["sha256"], f"inventory input pin differs: {record['path']}")
...
def _identity(root: Path, record: Any, pins: dict[str, str]) -> pd.DataFrame:
    manifest = pinned_object(_pin(root, record, pins), record["sha256"])
...
def _resolution(status: pd.Series, target: pd.Series, original: pd.Series, cohort: set[str],
                bridged: set[str]) -> pd.Series:
    """Query identity only: a cohort target via the bridge or an ID already in the cohort; never issuer relevance."""
    mapped = status.eq("mapped")
    return pd.Series(pd.NA, index=status.index, dtype="string").mask(mapped & target.isin(cohort), "bridged").mask(
        mapped & ~target.isin(cohort), "bridged_non_cohort").mask(~mapped & original.isin(cohort), "identity_equal").mask(
        ~mapped & ~original.isin(cohort) & original.isin(bridged), "outside_bridge_span").fillna("no_proven_identity")


def _records(unit: dict[str, Any], rows: pd.DataFrame, bridge: pd.DataFrame, cohort: set[str],
             bridged: set[str]) -> pd.DataFrame:
    ...
    mapped = map_news_relations(relations, bridge)
    ...
    resolution = _resolution(mapped.identity_translation_status, mapped.target_security_id,
                             mapped.identity_original_target_security_id, cohort, bridged)
...
def _inspect(root: Path, unit: dict[str, Any], bridge: pd.DataFrame, cohort: set[str],
             bridged: set[str]) -> tuple[pd.DataFrame | None, dict[str, Any]]:
    row = unit["ledger"]
    ...
    rows, summary = inspect_saved_alpaca_content(root=root, event_artifact=artifact, event_manifest=sidecar,
        ...
        memory_check=_guard)
    _parity(unit, summary)
    records = _records(unit, rows, bridge, cohort, bridged)
...
def _coverage(units: list[dict[str, Any]], summaries: dict[str, dict[str, Any]], bridge: pd.DataFrame,
              cohort: set[str], bridged: set[str]) -> pd.DataFrame:
    ...
    for archive in ARCHIVES:
        ...
        segments = map_news_coverage(requested[["chunk_id", "security_id", "ticker", "requested_start_utc", "requested_end_utc"]], bridge)
    ...
    result["query_identity_resolution"] = _resolution(result.identity_translation_status, result.security_id,
                                                      result.identity_original_security_id, cohort, bridged).to_numpy()
...
def _totals(records: pd.DataFrame, coverage: pd.DataFrame, summaries: dict[str, dict[str, Any]],
    ...
    mapped = records.loc[records.query_identity_resolution.eq("bridged") & records.inventory_status.eq("included"),
    ...
    return {"units": {f"{a}/{s}/{e}": int(n) for (a, s, e), n in units.groupby(["archive", "status", "evidence"]).size().items()},
        ...
        "bridged_included_records_outside_their_coverage_segment": exceptions}
...
def publish_cohort_content_inventory(*, root: Path, config: Path, config_sha256: str, output: Path,
    ...
    with heavy_job_lease("inspect-issuer-content-cohort", runtime_dir=runtime, config_path=config):
        _guard()
        pins: dict[str, str] = {}
        settings = pinned_object(_pin(root, {"path": config.relative_to(root).as_posix(), "sha256": config_sha256}, pins),
                                 config_sha256)
        _require(set(settings) == {"schema", "monthly_news_config", "identity_manifest", "approved_population"}
                 and settings["schema"] == CONFIG_SCHEMA, "cohort inventory configuration differs")
        monthly = pinned_object(_pin(root, settings["monthly_news_config"], pins), settings["monthly_news_config"]["sha256"])
        ...
        bridge = _identity(root, settings["identity_manifest"], pins)
        population = _pin(root, settings["approved_population"], pins)
        cohort_ids = load_swing_research_cohort(population, source_root=root).retained_security_ids
        cohort, bridged = set(cohort_ids), set(bridge.source_security_id)
        ...
        for name in ARCHIVES:
            ...
            for index in range(0, len(selected), PART_CHUNKS):
                ...
                if part in parts:
                    ...
                    frames, saved = [], []
                    for unit in batch:
                        records, summary = _inspect(root, unit, bridge, cohort, bridged)
        ...
        coverage = _coverage(units, summaries, bridge, cohort, bridged)
