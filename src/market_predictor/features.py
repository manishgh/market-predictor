...
def build_event_swing_dataset(
    ...
    *,
    ...
    """Build one row per timestamped event/news item.

    Pre-event trend features are taken from the last completed daily bar before
    the event's actionable date. Reaction features are explicitly named as such
    because they are only available after the event has had time to trade.
    """
    events = pd.read_parquet(events_path)
    if events.empty:
    ...
    daily = add_price_features(daily.sort_values("date"))
    daily = add_benchmark_features(ticker, daily, settings, start, end)
    events = align_events_to_trading_dates(events, daily["date"])
    if events.empty:
        raise ValueError(f"No events in {events_path} matched an available trading candle for {ticker}.")
    market_context = load_market_context_daily(market_context_path)
    if not market_context.empty:
        daily = daily.merge(market_context, on="date", how="left")
        for col in market_context.columns:
            if col != "date":
                daily[col] = daily[col].fillna(0.0)
    daily = _add_event_forward_labels(daily, horizon_days=horizon_days)
    daily_by_date = daily.set_index("date", drop=False)
    daily_dates = list(daily_by_date.index)

    hourly_start = events["timestamp"].min().to_pydatetime() - timedelta(days=7)
    hourly = fetch_hourly_prices(ticker, hourly_start, end, settings)
    if not hourly.empty:
    ...
    for event in events.sort_values("timestamp").itertuples(index=False):
        ...
        prev_day = _previous_daily_row(daily_by_date, daily_dates, event_date)
        if prev_day is None:
            continue
        sentiment = float(getattr(event, "sentiment_numeric", 0) or 0)
        title = str(getattr(event, "title", "") or "")
        text = str(getattr(event, "text", "") or "")
        title_has_ticker = _contains_ticker(title, ticker)
        text_has_ticker = _contains_ticker(f"{title} {text}", ticker)
        generic_headline = _is_generic_market_headline(title)
        record: dict[str, object] = {
            "ticker": ticker.upper(),
            "event_timestamp": event_ts,
            "date": event_date,
            "source": getattr(event, "source", ""),
            "source_family": getattr(event, "source_family", ""),
            "title": title,
            "event_time_bucket": getattr(event, "event_time_bucket", ""),
            "sentiment_numeric": sentiment,
            "sentiment_abs": abs(sentiment),
            "title_has_ticker": float(title_has_ticker),
            "text_has_ticker": float(text_has_ticker),
            "generic_movers_headline": float(generic_headline),
            "event_relevance_score": _event_relevance_score(
                title_has_ticker=title_has_ticker,
                text_has_ticker=text_has_ticker,
                generic_headline=generic_headline,
                source_family=str(getattr(event, "source_family", "") or ""),
            ),
            "engagement_score": float(getattr(event, "engagement_score", 0) or 0),
            "engagement_comments": float(getattr(event, "engagement_comments", 0) or 0),
            "engagement_upvote_ratio": float(getattr(event, "engagement_upvote_ratio", 0) or 0),
            "sector_name": event_day.get("sector_name", settings.sector_for_ticker(ticker) or "unknown"),
            "sector_benchmark": event_day.get("sector_benchmark", settings.sector_benchmark_for_ticker(ticker)),
            "market_benchmark": event_day.get("market_benchmark", settings.market_benchmark_ticker),
            "event_day_gap_pct": _safe_float(event_day.get("gap_pct")),
            "event_day_open_to_close_return": _safe_ratio(event_day.get("close"), event_day.get("open")),
            "event_day_close_vs_prev_close": _safe_float(event_day.get("return_1d")),
            "next_1d_return": _safe_float(event_day.get("event_next_1d_return"), default=np.nan),
            f"next_{horizon_days}d_return": _safe_float(
                event_day.get(f"event_next_{horizon_days}d_return"),
                default=np.nan,
            ),
        ...
        for name in EVENT_KEYWORDS:
            record[f"event_{name}"] = float(getattr(event, f"event_{name}", 0) or 0)
        for col in EVENT_PRE_DAILY_FEATURES:
            record[f"pre_{col}"] = _safe_float(prev_day.get(col))
        record.update(_hourly_event_features(hourly, event_ts))
        record["reaction_2h_x_sentiment"] = _safe_float(record.get("reaction_2h")) * sentiment
        record["reaction_abs_2h_x_sentiment_abs"] = _safe_float(record.get("reaction_abs_2h")) * abs(sentiment)
        record["event_gap_x_sentiment"] = _safe_float(record.get("event_day_gap_pct")) * sentiment
        rows.append(record)
    ...
    return frame.sort_values(["event_timestamp", "ticker"]).reset_index(drop=True)


EVENT_PRE_DAILY_FEATURES = [
    "return_1d",
    "return_5d_past",
    "return_10d_past",
    "return_20d_past",
...
def _add_event_forward_labels(daily: pd.DataFrame, *, horizon_days: int) -> pd.DataFrame:
    frame = daily.copy()
    next_open = frame["open"].shift(-1)
    next_close = frame["close"].shift(-1)
    horizon_close = frame["close"].shift(-horizon_days)
    frame["event_next_1d_return"] = next_close / next_open - 1.0
    frame[f"event_next_{horizon_days}d_return"] = horizon_close / next_open - 1.0
    frame["target_event_next_1d_up"] = (frame["event_next_1d_return"] > 0).astype("float")
    frame[f"target_event_next_{horizon_days}d_up"] = (frame[f"event_next_{horizon_days}d_return"] > 0).astype("float")
    frame.loc[frame.index[-1:], ["event_next_1d_return", "target_event_next_1d_up"]] = np.nan
    tail = frame.index[-horizon_days:]
    frame.loc[tail, [f"event_next_{horizon_days}d_return", f"target_event_next_{horizon_days}d_up"]] = np.nan
    return frame


def _previous_daily_row(daily_by_date: pd.DataFrame, daily_dates: list[date], event_date: date) -> pd.Series | None:
    try:
    ...
    if idx <= 0:
        return None
    return daily_by_date.loc[daily_dates[idx - 1]]


def _hourly_event_features(hourly: pd.DataFrame, event_ts: pd.Timestamp) -> dict[str, float]:
    empty = {
        "pre_1h_return": np.nan,
        "pre_4h_return": np.nan,
        "reaction_1h": np.nan,
        "reaction_2h": np.nan,
        "reaction_4h": np.nan,
        "reaction_abs_2h": np.nan,
        "reaction_volume_2h": np.nan,
        "first_bar_delay_minutes": np.nan,
    }
    if hourly.empty:
        return empty
    after = hourly[hourly["timestamp"] >= event_ts]
    if after.empty:
        return empty
    first = after.iloc[0]
    base = float(first["open"]) if pd.notna(first["open"]) and float(first["open"]) != 0 else np.nan
    if pd.isna(base):
        return empty
    result = dict(empty)
    result["first_bar_delay_minutes"] = float((pd.Timestamp(first["timestamp"]) - event_ts).total_seconds() / 60.0)
    for hours, name in [(1, "pre_1h_return"), (4, "pre_4h_return")]:
        before = hourly[hourly["timestamp"] <= event_ts - pd.Timedelta(hours=hours)]
        latest_before = hourly[hourly["timestamp"] <= event_ts]
        if not before.empty and not latest_before.empty:
            start_close = float(before.iloc[-1]["close"])
            end_close = float(latest_before.iloc[-1]["close"])
            result[name] = end_close / start_close - 1.0 if start_close else np.nan
    for hours, name in [(1, "reaction_1h"), (2, "reaction_2h"), (4, "reaction_4h")]:
        window = hourly[(hourly["timestamp"] >= event_ts) & (hourly["timestamp"] <= event_ts + pd.Timedelta(hours=hours))]
        if not window.empty:
            result[name] = float(window.iloc[-1]["close"]) / base - 1.0
    result["reaction_abs_2h"] = abs(result["reaction_2h"]) if pd.notna(result["reaction_2h"]) else np.nan
    volume_window = hourly[(hourly["timestamp"] >= event_ts) & (hourly["timestamp"] <= event_ts + pd.Timedelta(hours=2))]
    if not volume_window.empty:
        result["reaction_volume_2h"] = float(volume_window["volume"].sum())
    return result
...
def _safe_ratio(numerator: object, denominator: object) -> float:
    num = _safe_float(numerator, default=np.nan)
    den = _safe_float(denominator, default=np.nan)
