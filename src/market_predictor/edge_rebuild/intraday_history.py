...
def build_intraday_history_plan(
    *,
    ...
    memberships, membership_identity = _verify_memberships(
        memberships_path,
        membership_audit_path,
        minimum_cross_section=config.minimum_session_cross_section,
    )
    stock_identity = _verify_existing_ohlcv(
        existing_stock_bars_directory,
        config=config,
    )
    benchmark_identity = _verify_existing_ohlcv(
    ...
    plan_fingerprint = _json_sha256(request)
    ...
    try:
        ...
        _write_json(temporary / "_authority.json", authority)
...
def load_complete_intraday_history_plan(directory: Path) -> dict[str, Any]:
    ...
    if (
        not fingerprint
        or _json_sha256(unhashed_request) != fingerprint
        or manifest.get("schema") != INTRADAY_HISTORY_PLAN_SCHEMA
        ...
        raise DataReadinessError(
            "ER1A intraday-history plan lacks matching complete authority"
        )
...
def _build_plan_frames(
    *,
    ...
    for session in sessions:
        ...
        if len(active) < config.minimum_session_cross_section:
            raise DataReadinessError(
        ...
        all_tickers.update(active["ticker"].astype(str))
        symbols = sorted(set(active["ticker"].astype(str)).union(
            benchmark_tickers
        ))
        session_minutes = int((close_at - open_at).total_seconds() // 60)
        expected_bars_per_symbol = (session_minutes + 4) // 5
        symbols_per_unit = min(
            config.maximum_symbols_per_unit,
            config.maximum_expected_rows_per_unit
            // expected_bars_per_symbol,
        )
        if symbols_per_unit < 1:
            raise DataReadinessError(
                f"ER1A unit row cap cannot fit {session.date()}"
            )
        for offset in range(0, len(symbols), symbols_per_unit):
            chunk = symbols[offset : offset + symbols_per_unit]
            mapping = {
                provider_symbol(ticker, "alpaca"): ticker
                for ticker in chunk
            }
            if len(mapping) != len(chunk):
                raise DataReadinessError(
                    f"ER1A provider-symbol collision on {session.date()}"
                )
            expected_rows = expected_bars_per_symbol * len(chunk)
