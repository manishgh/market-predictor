...
CAUSAL_INTRADAY_MODEL_FEATURE_COLUMNS: Final = (
    ...
    "ema_10_distance",
    "ema_10_distance_zscore",
    "ema_20_distance",
    "sma_10_distance",
    "sma_10_distance_zscore",
    "sma_20_distance",
    "macd",
    "macd_zscore",
    "macd_signal",
    ...
    "session_vwap_distance_atr_zscore",
    "volume_bar_progress",
...
def build_causal_intraday_features(
    ...
    *,
    ...
    pipeline = FeaturePipeline([
        ...
        IntradaySessionContextStep(contract, FEATURE_SCHEMA_VERSION),
    ])
...
def _minute_context(frame: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, group in frame.groupby(["ticker", "session_date_et"], sort=False, observed=True):
        ...
        local_time = part["bar_start_utc"].dt.tz_convert(EXCHANGE_TIMEZONE)
        is_first_15m = (local_time.dt.time >= time(9, 30)) & (local_time.dt.time < time(9, 45))
        is_after_or = local_time.dt.time >= time(9, 45)
        
        or_high_val = part.loc[is_first_15m, "high"].max()
        or_low_val = part.loc[is_first_15m, "low"].min()
        
        part["distance_to_15m_high"] = np.where(is_after_or, (part["close"] / or_high_val - 1.0), np.nan).astype("float32")
        part["distance_to_15m_low"] = np.where(is_after_or, (part["close"] / or_low_val - 1.0), np.nan).astype("float32")
    ...
    *,
    ...
    renamed = context.rename(
        columns={
            ...
            "distance_to_15m_high": f"{prefix}_distance_to_15m_high",
            "distance_to_15m_low": f"{prefix}_distance_to_15m_low",
