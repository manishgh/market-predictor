...
class Settings(BaseSettings):
    ...
    def has_azure_storage(self) -> bool:
        return bool(self.azure_storage_connection_string_value or self.azure_storage_account_url)
    ...
    def sector_benchmarks(self) -> dict[str, str]:
        return {str(sector): str(ticker).upper() for sector, ticker in dict(config_get(self.app_config, "sector_benchmarks", {})).items()}

    @property
    def sector_ticker_groups(self) -> dict[str, list[str]]:
        groups = dict(config_get(self.app_config, "sector_tickers", {}))
        return {str(sector): [str(ticker).upper() for ticker in tickers] for sector, tickers in groups.items()}
